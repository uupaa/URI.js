...
var REGEXP_PATH = /^([^ ?#]*)(?:(\?[^#]*))?(?:(#.*))?$/;
                ...
                //  [1]         [2]           [3]
var REGEXP_HAS_SCHEME = /^(https?|wss?|blob):/i;
var REGEXP_OMIT_SCHEME = /^\/\//;
...
URI["getExt"]     = URI_getExt;         // URI.getExt(url:URLString|URLObject):FileExtensionString
...
URI["hasScheme"]  = URI_hasScheme;      // URI.hasScheme(url:URLString):Boolean
...
//}@dev
    ...
    var fileExt = urlObject.pathname.split("/").slice(-1).join("");
...
function URI_hasScheme(url) { // @arg URLString
                              // @ret Boolean - true is absolute url
                              // @desc URL has scheme.
//{@dev
    if (VERIFY) {
        $valid($type(url, "String"), URI_hasScheme, "url");
    }
//}@dev

    return REGEXP_HAS_SCHEME.test(url);
}
...
//}@dev
    ...
    return REGEXP_HAS_SCHEME.test(url) ? REGEXP_URL.test(url)
...
//}@dev
    ...
    if (REGEXP_OMIT_SCHEME.test(url)) {
